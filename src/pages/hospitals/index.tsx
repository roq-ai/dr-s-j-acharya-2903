...
import { getHospitals, deleteHospitalById } from 'apiSdk/hospitals';
...
function HospitalListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<HospitalInterface[]>(
    ...
    () =>
      getHospitals({
        relations: ['user', 'appointment.count', 'service.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('hospital', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/hospitals/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('hospital', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/hospitals/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('hospital', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/hospitals/edit/${record.id}`} passHref legacyBehavior>
                          <Button
                      ...
                      {hasAccess('hospital', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                        <IconButton
                          onClick={(e) => {
                            e.stopPropagation();
