...
function ServiceListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('service', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/services/create`} passHref legacyBehavior>
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('hospital', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>hospital</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('hospital', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/hospitals/view/${record.hospital?.id}`}>
