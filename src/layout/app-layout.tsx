...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['doctor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<HospitalInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/hospitals` : null),
    () => getHospitals({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Hospital',
      path: isTenantUser ? `/hospitals/view/${data?.[0]?.id}` : '/hospitals',
      entity: 'hospital',
    ...
    { name: 'Service', path: '/services', entity: 'service', service: AccessServiceEnum.PROJECT, icon: FiSettings },
